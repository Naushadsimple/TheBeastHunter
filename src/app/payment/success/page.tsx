  ...
  useEffect(() => {
    ...
    const fetchOrderDetails = async () => {
      try {
        ...
        if (payError || !payment) {
          // If payment record not found, let's poll or wait a bit or try finding registration directly
          console.warn('Payment record not found, trying registration query...');
        }
        ...
        const { data: reg, error: regError } = await supabase
          .from('registrations')
          .select('*, event_id(*)')
          .eq('registration_code', orderId.replace('TBH-', 'REG-')) // code correlation fallback
          .maybeSingle();

        // If no registration correlation, let's query by user's latest registrations
        let finalReg = reg;
        if (!finalReg && payment) {
          const { data: correlatedReg } = await supabase
            ...
            .single();
          finalReg = correlatedReg;
        }

        if (!finalReg) {
          // Check if this is a purely mock session where database webhook hasn't fired yet
          // In local mock testing, we can manually trigger the database update if statusParam is success
          if (statusParam === 'success' && payment) {
            // Force verify/update client-side for sandbox convenience
            await supabase
              .from('payments')
        ...
        // Set status
        const isPaid = finalReg.payment_status === 'paid' || finalReg.status === 'confirmed';
        setSuccess(isPaid);
  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center space-y-4">
        ...
        <h2 className="font-bebas text-2xl tracking-widest text-white uppercase animate-pulse">Verifying Payment</h2>
      </div>
    );
  }
  ...
  if (!success) {
    return (
      <div className="min-h-[70vh] max-w-md mx-auto px-4 flex flex-col items-center justify-center text-center space-y-6">
        ...
        <div className="space-y-2">
          <h2 className="font-bebas text-4xl text-white tracking-wider uppercase">Payment Failed</h2>
          <p className="text-gray-400 text-sm font-inter">
            Your transaction for <strong>{eventData?.title}</strong> could not be completed. No money was charged, or if charged, it will be refunded.
          </p>
        </div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12 space-y-12">
      ...
      <div className="text-center space-y-4 no-print">
        <div className="inline-flex w-16 h-16 rounded-full bg-gold-premium/10 border border-gold-premium/30 items-center justify-center text-gold-premium mb-2">
      ...
      <div 
        id="printable-ticket" 
        className="bg-[#0B0B0B] border border-gold-premium/30 rounded-xl overflow-hidden relative shadow-[0_0_40px_rgba(212,175,55,0.15)] flex flex-col md:flex-row transition-all duration-300 hover:border-gold-premium/60"
      >
        {/* Shimmer Effect */}
          ...
          <div className="flex items-start justify-between">
            <div className="space-y-1">
              <span className="font-bebas text-xs uppercase tracking-widest text-gold-premium font-bold flex items-center space-x-1.5">
                ...
                <span>The Beast Hunter Arena</span>
              </span>
